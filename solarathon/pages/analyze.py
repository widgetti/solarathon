...
def plotTA(ticker, days_range, returns, volume, bbanduplow, bbandmidd, ema10, ema30):
    ...
    if ema10.value:
        ...
        addplots.append(
            mpf.make_addplot(
                df["EMA10"], color="blue", secondary_y=False, label="EMA10"
            )
        )
    ...
    if bbanduplow.value:
        ...
        addplots.append(
            mpf.make_addplot(
                df["BBL_5_2.0"], color="purple", secondary_y=False, label="Low BBand"
            )
        )
        addplots.append(
            mpf.make_addplot(
                df["BBU_5_2.0"], color="purple", secondary_y=False, label="Up BBand"
            )
        )
    ...
    if bbandmidd.value:
        ...
        addplots.append(
            mpf.make_addplot(
                df["BBM_5_2.0"], color="orange", secondary_y=False, label="Mid BBand"
            )
        )
    ...
    if ema30.value:
        ...
        addplots.append(
            mpf.make_addplot(df["EMA30"], color="red", secondary_y=False, label="EMA30")
        )
    ...
    style = mpf.make_mpf_style(
        base_mpf_style="yahoo", marketcolors=mpf.make_marketcolors(up="g", down="r")
    )
    ...
    fig, axes = mpf.plot(
        ...
        type="candle",
        style=style,
        title=f"Candlestick and TA - {ticker}",
        ylabel="Price",
        addplot=addplots,
        figsize=(12, 8),
        returnfig=True,
...
def Page():
    ...
    with solara.Sidebar():
        with solara.Column(align="stretch"):
            solara.Select(
                label="Select Crypto asset:", value=crypto, values=crypto_list
            )
    ...
    if message == "Download_successful":
        solara.Success(
            f"Asset selected: {crypto} {message}",
            text=True,
            dense=True,
            outlined=True,
            icon=True,
